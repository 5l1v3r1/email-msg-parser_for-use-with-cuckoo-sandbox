...
class email_attachment(object):
    def __init__(self, name, abs_filepath, parent_msg_filename):
        self.name = name
...
if __name__ == "__main__":
    ...
    # Get all .msg filenames    
    current_working_directory = os.getcwd()
    msg_files_directory = os.path.join(current_working_directory, "msg-files")
    msg_files = os.listdir(msg_files_directory)
    
    email_attachments_to_analyze = []
    for msg_file in msg_files:       
        msg_file_path = os.path.join(msg_files_directory, msg_file)
        email_msg = OleFile.OleFileIO(msg_file_path)
...
#         except Exception:
        ...
        for attachmentDir in attachmentDirs:
            ...
            # Get parent .msg filename
            parentMsgFilename = msg_file
            # Get attachment data
            data = _getStream(email_msg, [attachmentDir, '__substg1.0_37010102'])
            
            # Create new attachment object
            attachment_obj = type('obj', (object,), {'longFilename' : longFilename, 'shortFilename' : shortFilename, 'parentMsgFilename' : parentMsgFilename, 'data' : data})
            
            # Append new attachment object
            emailMsgAttachments.append(attachment_obj)
        ...
        for emailMsgAttachment in emailMsgAttachments:    
            ...
            email_attachments_to_analyze.append(email_attachment(filename, absolute_filename, emailMsgAttachment.parentMsgFilename))
    ...
    for email_attachment in email_attachments_to_analyze:             
        REST_URL = "http://localhost:9001/tasks/create/file"
        
    
        print "REST_URL is: " + REST_URL
        print "File sending to Cuckoo Sandbox is: " + email_attachment.name
        
        with open(email_attachment.abs_filepath, "rb") as sample:
            files = {"file": (email_attachment.name, sample)}
            r = requests.post(REST_URL, files=files)
        
        task_id = r.json()["task_id"]
        report_url = "http://localhost:9001/tasks/report/" + str(task_id)
        
        print "File sent to Cuckoo Sandbox is: " + email_attachment.name + ", task_id (Cuckoo Sandbox id) is: " + str(task_id) + ", report_url is: " + report_url
        
        # Add report on email attachment to the report list
        email_attachment_report_list.append(email_attachment_report(email_attachment.name, email_attachment.parent_msg_filename, report_url))
